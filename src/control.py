...
def run():
    ...
    while True:
        ...
        r = requests.post("http://greenalytics.ga:5000/api/hardware/temperature", json=paramdict)
        print(r.status_code)

        paramdict.update({"UTCTime": round(time.time()), "SensorValue": round(humidity)})
        r = requests.post("http://greenalytics.ga:5000/api/hardware/humidity", json=paramdict)
        print(r.status_code)

        paramdict.update({"UTCTime": round(time.time()), "SensorValue": lightlevel})
        r = requests.post("http://greenalytics.ga:5000/api/hardware/light", json=paramdict)
        print(r.status_code)
